...
bool MemOpRatioLogger::runOnFunction(Function &f) {
    ...
    for (auto &i: insts) {
        if (isa<LoadInst>(i) || isa<StoreInst>(i) || isa<ReturnInst>(i) || isa<BinaryOperator>(i)) {
            visit(i);
        }
    }
    return true;
...
void MemOpRatioLogger::visitLoadInst(LoadInst &inst) {
    ...
    if (ptr_op) {
        ptr_is_input = inputs.count(ptr_op) > 0 || std::count(args.begin(), args.end(), ptr_op);
        if (ptr_is_input)
            inputs.insert(ptr_op);
    }
    if (isa<GetElementPtrInst>(ptr_op)) {
        gep = dyn_cast<GetElementPtrInst>(ptr_op);
        gep_is_input = std::count(args.begin(), args.end(), gep->getPointerOperand()) || inputs.count(gep->getPointerOperand());
        if (gep_is_input) {
...
void MemOpRatioLogger::visitStoreInst(StoreInst &inst) {
    ...
    if (inst.getPointerOperand()->getNameOrAsOperand().find("cache") == std::string::npos) {
...
static RegisterPass<MemOpRatioLogger> X("mem-op-ratio", "MemOp Ratio Logger Pass");
