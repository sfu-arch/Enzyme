    ...
    for (auto &BB: f) {
        ...
        for (auto &I: BB) 
        {
            if (loop_id_map.count(BB.getName()))
                I.setMetadata("LoopId", MDNode::get(I.getContext(), MDString::get(I.getContext(), std::to_string(loop_id_map[BB.getName()]))));
            insts.push_back(&I);
        }
    }
    ...
    for (auto &arg: f.args()) {
        for (auto user=arg.user_begin(); user != arg.user_end(); user++) {
            if (IsReverseOp(*user) && isa<Instruction>(*user)) {
                auto *load = new LoadInst(Type::getInt32Ty(f.getContext()), reverse_op, "augmented_load", cast<Instruction>(*user));
                std::string line = "-OP: " + user->getNameOrAsOperand() + ", " + arg.getNameOrAsOperand() + ", %d\n";
...
void OPCounterPass::visitLoadInst(LoadInst &I) {
    ...
    log_info += I.getParent()->getName().contains("invert") ? "REVERSE" : "FORWARD";
    unique_tag += I.getParent()->getName().contains("inv") ? "_rev_" : "_fw_";
    if (I.hasMetadata("LoopId")) {
        log_info += ", LoopId, " + dyn_cast<MDString>(I.getMetadata("LoopId")->getOperand(0))->getString().str();
        unique_tag += "_loop";
    }
    log_info += "\n";
...
void OPCounterPass::UpdateOpCount(Instruction *I) {
    ...
    // auto *fw_load = new LoadInst(Type::getInt32Ty(I->getContext()), forward_op, "load_forward_count", store->getNextNode());
    // auto *depth = BinaryOperator::Create(Instruction::Add, inc, fw_load, "depth", fw_load->getNextNode());
    
    if (isa<BinaryOperator>(I)) {
        if (!isa<Constant>(I->getOperand(0))) {
            std::string to_print = "-OP: " + I->getNameOrAsOperand() + ", " + I->getOperand(0)->getNameOrAsOperand() + ", %d\n";
            CallPrintf(I, &to_print[0], {inc}, to_print);
        }
        if (!isa<Constant>(I->getOperand(1))) {
            std::string to_print = "-OP: " + I->getNameOrAsOperand() + ", " + I->getOperand(1)->getNameOrAsOperand() + ", %d\n";
            CallPrintf(I, &to_print[0], {inc}, to_print);
        }
    }
...
void OPCounterPass::visitReturnInst(ReturnInst &I) {
    ...
    CallPrintf(forward_mem_load, 
               ...
               {forward_op_load, reverse_op_load, forward_mem_load, reverse_mem_load}, "ops_log");
}
