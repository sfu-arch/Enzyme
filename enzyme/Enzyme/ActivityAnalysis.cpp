...
bool ActivityAnalyzer::isConstantValue(TypeResults &TR, Value *Val) {
  ...
  if (containsPointer) {
    ...
    for (BasicBlock &BB : *TR.getFunction()) {
      ...
      if (IVal && IVal->getParent() != &BB && TR.analyzer.DT->dominates(&BB, IVal->getParent())) {
        continue;
      }
      for (Instruction &I : BB) {
        ...
        if (IVal && IVal->getParent() == &BB && (IVal == &I || TR.analyzer.DT->dominates(&I, IVal))) {
          continue;
        }
          ...
          if (isa<LoadInst>(&I) ||
            ...
            if (!Hypothesis->isConstantValue(TR, &I)) {
                ...
                if (TR.query(&I)[{-1}].isPossiblePointer()) {
                    if (EnzymePrintActivity)
                      llvm::errs() << "potential active store via pointer in load: " << I << " of " << *Val << "\n";
                    potentialStore = true;
                }
            }
            ...
            if (!Hypothesis->isConstantValue(TR, MTI->getArgOperand(0))) {
                ...
                if (TR.query(Val)[{-1, -1}].isPossiblePointer()) {
                    if (EnzymePrintActivity)
                      llvm::errs() << "potential active store via pointer in memcpy: " << I << " of " << *Val << "\n";
                    potentialStore = true;
                }
            }
          } else {
            ...
            if (!Hypothesis->isConstantInstruction(TR, &I) ||
                ...
                if (TR.query(Val)[{-1, -1}].isPossiblePointer()) {
                    // If this instruction either can store into the inner pointer, or could return
                    // an active loaded pointer(thus into a potential pointer of pointer
                    if (I.mayWriteToMemory() || (!Hypothesis->isConstantValue(TR, &I) && TR.query(&I)[{-1}].isPossiblePointer())) {
                      if (EnzymePrintActivity)
                        llvm::errs() << "potential active store via pointer in unknown inst: " << I << " of " << *Val << "\n";
                      potentialStore = true;
                    }
                }
            }
        ...
        if (!potentialStore && isModSet(AARes)) {
          ...
          if (auto SI = dyn_cast<StoreInst>(&I)) {
            ...
            if (EnzymePrintActivity)
                llvm::errs() << " -- store potential activity: " << (int)cop << " - " << *SI << " of " << " Val=" << *Val << "\n";
            potentialStore |= cop;
          } else if (auto MTI = dyn_cast<MemTransferInst>(&I)) {
            potentialStore |=
...
bool ActivityAnalyzer::isInstructionInactiveFromOrigin(TypeResults &TR,
  ...
  if (auto SI = dyn_cast<StoreInst>(inst)) {
    ...
    if (isConstantValue(TR, SI->getValueOperand()) ||
      ...
      if (EnzymePrintActivity)
        llvm::errs() << " constant instruction as store operand is inactive " << *inst << "\n";
      return true;
    }
  }
...
bool ActivityAnalyzer::isValueActivelyStoredOrReturned(TypeResults &TR,
  ...
  if (EnzymePrintActivity)
    llvm::errs() << " <ASOR" << (int)directions << " ignoreStoresinto=" << ignoreStoresInto << ">" << *val << "\n";
  ...
  for (const auto a : val->users()) {
    ...
    if (isa<ReturnInst>(a)) {
      ...
      if (EnzymePrintActivity)
        llvm::errs() << " </ASOR" << (int)directions << " ignoreStoresInto=" << ignoreStoresInto << ">" << " active from-ret>"
                     << *val << "\n";
      ...
      if (SI->getValueOperand() != val) {
        if (!ignoreStoresInto) {
          ...
          if (!isConstantValue(TR, SI->getValueOperand())) {
            StoredOrReturnedCache[key] = true;
            if (EnzymePrintActivity)
              llvm::errs() << " </ASOR" << (int)directions << " ignoreStoresInto=" << ignoreStoresInto << " active from-store>"
                           << *val << " store into=" << *SI << "\n";
            return true;
          }
        }
    ...
    if (EnzymePrintActivity)
      llvm::errs() << " </ASOR" << (int)directions << " ignoreStoresInto=" << ignoreStoresInto << " active from-unknown>"
                   << *val << " - use=" << *a << "\n";
    return StoredOrReturnedCache[key] = true;
  }

  if (EnzymePrintActivity)
    llvm::errs() << " </ASOR" << (int)directions << " ignoreStoresInto=" << ignoreStoresInto << " inactive>" << *val
                 << "\n";
  return false;
}
