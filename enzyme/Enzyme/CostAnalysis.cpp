  ...
  for (auto &BB : f) {
    for (auto &I : BB) {
      ...
      if (BB.getName().contains("invert") || isa<AllocaInst>(I)) {
        I.setMetadata("mode",
                      MDNode::get(context, MDString::get(context, "reverse")));
        // I.setMetadata("tapeCost", MDNode::get(context, MDString::get(context,
        // "0")));
      } else
        I.setMetadata("mode",
                      MDNode::get(context, MDString::get(context, "forward")));
...
void InstruMemPass::visitBinaryOperator(BinaryOperator &ins) {
  ...
  // ins.setMetadata("tapeCost", MDNode::get(ins.getContext(),
  // MDString::get(ins.getContext(), std::to_string(tapeCost))));
}

void InstruMemPass::visitStoreInst(StoreInst &ins) {

  auto *op1 = ins.getOperand(0);
  memOps[ins.getOperand(1)->getName().str()].first =
      std::max(memOps[ins.getOperand(1)->getName().str()].first,
               getLevel(ins.getOperand(0)));
  // memOps[ins.getOperand(1)->getName().str()].second =
  // std::max(memOps[ins.getOperand(1)->getName().str()].second,
  // getTapeCost(ins.getOperand(0)));

  ins.setMetadata(
      "level",
      MDNode::get(ins.getContext(),
                  MDString::get(ins.getContext(),
                                std::to_string(getLevel(ins.getOperand(0))))));
  // errs() << "Store: " << ins.getOperand(1)->getName() << ": " <<
  // std::to_string(memOps[ins.getOperand(1)->getName().str()].first) << "\n";
}
  ...
  if (!ins.getParent()->getName().contains("invert"))
    for (auto i : ins.users()) {
      if (!isForwardNode(i))
        continue;
      if (isa<LoadInst>(i) ||
          (isa<StoreInst>(i) && !isa<Constant>(i->getOperand(0)))) {
        ins.setMetadata(
            "mode", MDNode::get(ins.getContext(),
                                MDString::get(ins.getContext(), "forward")));
        break;
      }
    }
  visitInstruction(ins);
...
void InstruMemPass::visitGetElementPtrInst(GetElementPtrInst &ins) {
  ...
  if (memOps.count(op1->getName().str())) {
    // ins.setMetadata("tapeCost", MDNode::get(ins.getContext(),
    // MDString::get(ins.getContext(),
    // std::to_string(memOps[op1->getName().str()].second))));
    ins.setMetadata(
        "level",
        MDNode::get(
            ins.getContext(),
            MDString::get(ins.getContext(),
                          std::to_string(memOps[op1->getName().str()].first))));
    // errs() << "Load: " << ins.getOperand(0)->getName() << ": " <<
    // std::to_string(memOps[op1->getName().str()].first) << "\n";
  } else
    visitInstruction(ins);
}
...
void InstruMemPass::visitInstruction(Instruction &ins) {
  ...
  ins.setMetadata(
      "level",
      MDNode::get(ins.getContext(),
                  MDString::get(ins.getContext(), std::to_string(maxLevel))));
}
