...
cl::opt<bool>
    ignore_reverse("ignore-reverse", cl::init(false), cl::Hidden,
...
void PrintFunctionArgs(Function &f) {
  ...
  for (int i = 0; i < f.arg_size(); i++) {
    ConstantInt *id =
        ConstantInt::get(Type::getInt32Ty(first_inst->getContext()), i + 1);
    std::string write_format = "F_Node: " + f.getArg(i)->getNameOrAsOperand() +
                               ", Parent: root, arg\n";
...
void LogCrossingEdge(Instruction *inst, std::ofstream &myfile) {
  if (!IsReverseOp(inst) || isa<LoadInst>(inst) || isa<StoreInst>(inst) ||
      isa<CallInst>(inst)) // Only considering reverse ops
    return;

  for (int i = 0; i < inst->getNumOperands(); i++) {
    if (isa<Instruction>(inst->getOperand(i)))
      if (!IsReverseOp(inst->getOperand(
              i))) // If I'm a reverse and I have a parent in the forward
...
bool NodeLogger::runOnFunction(Function &f) {
  ...
  for (auto i : insts)
    if (i && isa<Instruction>(i)) {
      if (!ignore_reverse ||
          !cast<Instruction>(i)->getParent()->getName().contains("invert")) {
...
void NodeLogger::visitLoadInst(LoadInst &inst) {
  ...
  if (!(inst.hasMetadata("read") || inst.hasMetadata("write")))
    return;
  std::string write_format = ModePrefix(&inst) +
                             "Node: " + inst.getNameOrAsOperand() +
                             ", Parent: %x, load\n";
...
void NodeLogger::visitStoreInst(StoreInst &inst) {
  ...
  if (isa<Constant>(inst.getValueOperand()))
    return;
  if (inst.getPointerOperand()->getNameOrAsOperand().find("'") !=
      std::string::npos)
    return;
  std::string write_format = ModePrefix(&inst) + "Node: %x, Parent: " +
                             inst.getValueOperand()->getNameOrAsOperand() +
                             ", store\n";
  // auto *value_id = ConstantInt::get(Type::getInt32Ty(inst.getContext()),
  // node_ids[inst.getValueOperand()]);
  CallPrintf(&inst, &write_format[0], {inst.getPointerOperand()}, write_format);
}

std::string GenerateWriteFormat(Instruction &inst,
  ...
  if (isa<Instruction>(inst)) {
    if (isa<BinaryOperator>(inst)) {
      ...
      if (isa<AllocaInst>(inst)) {
        ...
        write_format +=
            "arithmetic_" + std::string(inst.getOpcodeName()) + "\n";
      }
    } else {
      write_format += std::string(inst.getOpcodeName()) + "\n";
    }
...
void NodeLogger::visitInstruction(Instruction &inst) {
  ...
  if (isa<ReturnInst>(inst) || isa<BranchInst>(inst) ||
      isa<SelectInst>(inst) | isa<PHINode>(inst) || isa<AllocaInst>(inst))
    return;

  std::string write_format = "Node: " + inst.getNameOrAsOperand() + ", ";
  // ConstantInt *id = ConstantInt::get(Type::getInt32Ty(inst.getContext()),
  // node_ids[&inst]);
  std::vector<Value *> args = {};
  for (int i = 0; i < inst.getNumOperands(); i++) {
    Value *operand = inst.getOperand(i);
    if (operand && !isa<Constant>(operand)) {
      // args.push_back(ConstantInt::get(Type::getInt32Ty(inst.getContext()),
      // node_ids[operand]));
  ...
  CallPrintf(&inst, &write_format[0], {},
             write_format + std::to_string(node_ids[&inst]));
}
